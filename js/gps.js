...
async function persistPosition(position, { history = false } = {}) {
  ...
  const now = Date.now();
  const moved = distanceMeters(lastHistoryPosition, data);
  if (history || !lastHistoryPosition || now - lastHistoryAt >= HISTORY_INTERVAL_MS || moved >= HISTORY_DISTANCE_M) {
...
export function getGpsStatus() {
  ...
  return watchId === null ? "inactive" : "active";
}
...
export async function startTracking(user, onStatus = () => {}) {
  ...
  watchId = navigator.geolocation.watchPosition(async position => {
    try {
      ...
      emit("error", { message: error?.message || "No se pudo guardar la ubicación." });
    }
  }, error => {
    emit("error", { message: normalizeError(error) });
  }, {
